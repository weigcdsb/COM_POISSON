function [theta,W,Q, lam_pred,nu_pred,log_Zvec_pred,...
    lam_filt,nu_filt,log_Zvec_filt,...
    lam_smoo,nu_smoo,log_Zvec_smoo] =...
    ppasmoo_compoisson_v2_window_fisher_Qpart(theta0, N,X_lam,G_nu,W0,F,...
    windSize, windType, Qpart, varargin)
...
options = optimset('DiffMinChange',DiffMinChange,'DiffMaxChange',DiffMaxChange,...
    'MaxFunEvals', MaxFunEvals, 'MaxIter', MaxIter, 'Display', 'off');
...
for i=2:n_spk
    ...
    if((i > (QpartIdx-1)*round(n_spk/Qpart)+1) && QoptTag == false)
        Qidx = ((QpartIdx-1)*round(n_spk/Qpart)+1):min(QpartIdx*round(n_spk/Qpart), n_spk);
        f = @(Q) helper_window(Q, theta(:, Qidx(1)), N(Qidx),...
            X_lam(Qidx,:),G_nu(Qidx, :),W(:,:,Qidx(1)),F,1,windType);
        [~, Qopt] = evalc('fmincon(f,Qopt0,[],[],[],[],[QLB QLB],[QUB QUB], [], options)');
        Q(:,:,Qidx) = zeros(size(Q(:,:,Qidx))) + diag(Qopt);
        QpartIdx = QpartIdx + 1;
    end
    
    thetapred(:,i) = F*theta(:,i-1);
    Wpred(:,:,i) = F*W(:,:,i-1)*F' + Q(:, :, i-1);
