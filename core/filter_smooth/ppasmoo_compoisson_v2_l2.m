function [theta,W] = ppasmoo_compoisson_v2_l2(theta0, N,X_lam,G_nu,W0,F,Q,zeta)
...
%     var_Y = exp(log(B) - log(Z)) - mean_Y^2;
%     mean_logYfac = exp(log(C) - log(Z));
%     var_logYfac = exp(log(D) - log(Z)) - mean_logYfac^2;
%     cov_Y_logYfac =  exp(log(E)-log(Z))-exp(log(A)+log(C)-2*log(Z));
    ...
    Wpostinv = inv(Wpred(:,:,i)) + [info1, info2; info3, info4] -...
        2*zeta*eye(length(theta0));
    W(:,:,i) = inv(Wpostinv); % penalized term
    
    theta(:,i)  = thetapred(:,i) +...
        W(:,:,i)*([(sum(N(:, i))- nCell*mean_Y)*X_lam(i,:)';...
        nu(i)*(-sum_logfac + nCell*mean_logYfac)*G_nu(i, :)'] + ...
        2*zeta*thetapred(:,i)); % penalized term
