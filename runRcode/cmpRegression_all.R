...
y <- read.csv(paste0(wd, '/yAll.csv'), header = F)
X <- read.csv(paste0(wd, '/XAll.csv'), header = F)
G <- read.csv(paste0(wd, '/GAll.csv'), header = F)
...
write.csv(c(beta, gam),
          file = paste0(wd, '/cmp_all.csv'), row.names = F)