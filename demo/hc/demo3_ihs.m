...
nSub = round(length(spk)/3);

f = @(Q) helper_window_v2(Q, theta02, spk(1:nSub)',X(1:nSub, :),G(1:nSub, :),...
...
fWind = @(windSize) helper_window_v2_windSize(windSize, Qoptmatrix,...
    theta02, spk(1:nSub)', X(1:nSub, :), G(1:nSub, :), W02,...
...
plot(t_raw,pos)
hold on
% plot(t_raw(find(spk_raw>0)),pos(spk_raw>0),'k.')
...
subplot(3,1,2)
imagesc(t_raw(1:skip:end),heatmap_pos*500-250,heatmap_mean)
box off; set(gca,'TickDir','out')
set(gca,'YDir','normal')
ylabel('Mean')
...
subplot(3,1,3)
imagesc(t_raw(1:skip:end),heatmap_pos*500-250,log10(heatmap_var./heatmap_mean))
box off; set(gca,'TickDir','out')
set(gca,'YDir','normal')
