...
QLB = 1e-8;
QUB = 1e-3;
Q0 = [1e-3*ones(1, length(theta0)) 0];
DiffMinChange = QLB;
DiffMaxChange = QUB*0.1;
MaxFunEvals = 200;
MaxIter = 100;

f = @(Q) helper_window_v4(Q, theta0, spk_vec,X_lam,G_nu,W0,F,1, windType);
...
Qmatrix = diag(Qopt(1:(end - 1)));
Qmatrix(1, size(X_lam, 2)+1) = Qopt(end);
Qmatrix(size(X_lam, 2)+1, 1) = Qopt(end);
