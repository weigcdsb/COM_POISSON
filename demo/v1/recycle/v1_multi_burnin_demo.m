...
[theta_fit2,W_fit2] =...
    ppafilt_compoisson_v3(theta0, ry_aug,repmat(X, [1 1 size(ry_aug, 2)]),repmat(G, [1 1 size(ry_aug, 2)]),...
    eye(length(theta0)),eye(length(theta0)),Q);

theta_fit2 = theta_fit2(:, (repTrial + 1):end);
W_fit2 = W_fit2(:, :, (repTrial + 1):end);
...
theta_fit = repelem(theta_fit2,1,length(theta));
