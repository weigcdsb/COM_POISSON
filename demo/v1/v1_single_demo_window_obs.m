...
for i=1:size(data.EVENTS, 2)-smoothing
    ...
    c=1;
    for j = theta_idx
        tid = [j:100:(100*smoothing)]+i*100;
        ff = getFF(trial_y(tid, neuron));
        obs_ff(c, i) = ff;
        obs_mean(c, i) = mean(trial_y(tid, neuron));
        c = c+1;
    end
...
ylabel('Fano Factor')
subplot(2,2,3)
imagesc([1 (120-smoothing)]+smoothing/2, [1 length(theta)], obs_mean)
colorbar()
title('Mean-obs')
subplot(2,2,4)
imagesc([1 (120-smoothing)]+smoothing/2, [1 length(theta)], obs_ff)
title('Fano Factor')
colorbar()
...
writematrix(repmat(X, length(trial), 1), 'C:\Users\gaw19004\Documents\GitHub\COM_POISSON\runRcode\X.csv')
writematrix(repmat(G, length(trial), 1), 'C:\Users\gaw19004\Documents\GitHub\COM_POISSON\runRcode\G.csv')
...
save('C:\Users\gaw19004\Desktop\COM_POI_data\v1_new_n72.mat')

%% compare to obs.: heatmap
